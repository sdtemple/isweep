...
def main():
    ...
    if roitab.shape[0] == 0:
        with open(fileout, 'w') as f:
            f.write('NAME\tCHROM\tMINZDIFFZ\tMAXZDIFFZ\tBPCENTER\tCMCENTER\tBPLEFTCENTER\tBPRIGHTCENTER\n')
        sys.exit()
    ...
    for i in range(roitab.shape[0]):
        ...
        tab['WEIGHT'] = tab['ZDIFFZ'] / tab['ZDIFFZ'].sum()
    ...
    finacol = ['NAME'] + initcol + ['ALPHA']
    roitab.sort_values(by='MAXDIFFZ', ascending=False, inplace=True)
    nrow = roitab.shape[0]
    roitab['NAME'] = [f'hit{i}' for i in range(1, nrow+1)]
    roitab['ALPHA'] = [0.95 for _ in range(1, nrow+1)]
    roitab = roitab[finacol]
    roitab.to_csv(fileout, sep='\t', index=False, columns=['NAME', 'CHROM', 'MINZDIFFZ', 'MAXZDIFFZ', 'BPCENTER', 'CMCENTER', 'BPLEFTCENTER', 'BPRIGHTCENTER', 'ALPHA'])
