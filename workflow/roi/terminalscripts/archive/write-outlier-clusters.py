...
#  i/o
short_ibd, fileout, K, scalar = sys.argv[1:]
K=int(float(K)/2)
...
def write_outliers(communities, fileout, scalar=5):
    ...
    for community in communities:
        if len(community) > cutoff:
            f = open(fileout + str(idx) + '.outlier.community','w')
...
# saving
write_outliers(communities, fileout, scalar)
